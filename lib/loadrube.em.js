...
function loadFixtureFromRUBE(body, fixtureJso) {    
    ...
    if (fixtureJso.hasOwnProperty('circle')) {
        ...
        if (fixtureJso.hasOwnProperty('customProperties')) {
            console.log("found some custom properties on fixture");
            fixtureJso.customProperties.forEach(function(prop){
                if (prop.name == "particleKill") {
                    fixture.SetUserData(6);
                }
                if (prop.name == "playerDamage") {
                    fixture.SetUserData(7);
                }
            });
        }
    }
...
function loadImageFromRUBE(imageJso, world, loadedBodies)
{
    ...
    image.center = parseVec(imageJso.center);
